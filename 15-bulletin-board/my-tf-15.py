...
class zWordCounter:
    def __init__(self, event_manager):
        ...
        self._event_manager.subscribe('print', self.print_z)
        self._z_words = [];

    def add(self, event):
        word = event[1]
        if (word.startswith('z') or word.startswith('Z')) and word not in self._z_words:
            self._z_words.append(word)

    def print_z(self, event):
        print self._z_words
#
# The main function
#
em = EventManager()
DataStorage(em), StopWordFilter(em), WordFrequencyCounter(em), zWordCounter(em)
